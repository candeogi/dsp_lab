...
err_lim = 0.0037; % -48.636 dB, to be used in DEV

% 1) design the filter using firpmord and firpm
F=[fp,fs];
A=[1,0];
DEV=[err_lim,err_lim];
...
% 2) plot the filter coefficients in time
figure
subplot(2,1,1)
t = T*(-N/2:N/2);
stem(t,h); grid; title('FIR - linear program approach - time domain')

% 3) show the filter frequency response 
